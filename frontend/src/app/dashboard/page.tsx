  ...
  useEffect(() => {
    ...
    apiFetch<{ agents: Agent[] }>('/agents?limit=100')
      .then(async (d) => {
        ...
        // Fetch stats for each owned agent
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className="p-5 rounded-xl border border-zinc-800/50 bg-zinc-900/30">
                  ...
                  <div className="text-3xl font-bold font-[var(--font-display)] text-[var(--celo-green)]">
                    {agents.length}
                  </div>
                </div>
                <div className="p-5 rounded-xl border border-zinc-800/50 bg-zinc-900/30">
                  <div className="text-xs text-zinc-500 mb-1">Total Revenue</div>
                  <div className="text-3xl font-bold font-[var(--font-display)] text-[var(--celo-gold)]">
                    {(totalRevenue / 1e18).toFixed(4)}
                    <span className="text-sm text-zinc-500 ml-1">cUSD</span>
                  </div>
                </div>
                <div className="p-5 rounded-xl border border-zinc-800/50 bg-zinc-900/30">
                  <div className="text-xs text-zinc-500 mb-1">Total API Calls</div>
                  <div className="text-3xl font-bold font-[var(--font-display)] text-[var(--celo-violet)]">
                    {totalCalls}
                  </div>
                </div>
              </div>
                ...
                <div className="rounded-xl border border-zinc-800/50 bg-zinc-900/30 overflow-hidden">
                  ...
                  <div className="divide-y divide-zinc-800/30">
                    {agents.map(agent => {
                      ...
                      return (
                        <div key={agent.id} className="px-5 py-4 flex items-center justify-between hover:bg-zinc-800/10 transition-colors">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              <Link href={`/agent/${agent.agentId}`} className="font-semibold text-sm hover:text-[var(--celo-green)] transition-colors">
                                {agent.name}
                              </Link>
                              <div className="text-[10px] text-zinc-600 font-mono">
                                #{agent.agentId} · {(Number(agent.pricePerCall) / 1e18).toFixed(2)} cUSD/call
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-6 shrink-0">
                            <div className="text-right hidden md:block">
                              ...
                              <div className="text-[10px] text-zinc-600">{((Number(s?.totalRevenue || 0)) / 1e18).toFixed(4)} cUSD</div>
                            </div>
